...
async function checkDetails() {
    console.log('--- branch_stock_movements details ---');
    const { data, error } = await supabase.rpc('run_dynamic_query', {
        query: "SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'branch_stock_movements'"
    });
    ...
    const { data: constraints, error: cError } = await supabase.rpc('run_dynamic_query', {
        query: "SELECT conname, pg_get_constraintdef(oid) as def FROM pg_constraint WHERE conrelid = 'branch_stock_movements'::regclass"
    });
