-- Move the financial effect of an item void from Stage 2 (manager/accountant
-- approval) to Stage 1 (cashier acknowledgment).
--
-- Why: the bill is already handed to the customer once the cashier
-- acknowledges the void, so the customer-facing total cannot wait for a
-- manager to approve later. Previously `void_order_item` only ran at manager
-- approval, so `pos_shift_orders.total_amount`/`balance_amount` stayed at
-- the pre-void amount while the cashier already had the bill in hand.
--
-- New flow:
--   Stage 1 (cashier ack)   -> cashier_acknowledge_item_void() now does the
--                              real financial void: mutates the item's
--                              voided_qty/active_qty/active_total, deducts
--                              the amount from total_amount/balance_amount.
--                              Audit log row is NOT written yet (the void
--                              isn't manager-approved yet).
--   Stage 2 (manager review) -> approve: clears void_pending_approval and
--                              writes the pos_item_void_log row (the
--                              append-only log keeps its existing meaning
--                              of "approved voids only").
--                              reject: reverses the Stage 1 financial
--                              change (adds the amount back) and reinstates
--                              the item.
--
-- `void_order_item` (the old single-stage RPC) is left in place, unused, for
-- rollback reference.
