...
async function sync() {
  ...
  try {
    ...
    const oldUbr = await oldPool.query(`SELECT * FROM user_branch_roles ORDER BY created_at`);
    console.log(`\nOld DB user_branch_roles: ${oldUbr.rows.length}`);

    // 6. Insert missing user_branch_roles into new DB
    // First, get existing UBR in new DB
    ...
    for (const ubr of oldUbr.rows) {
      ...
      if (!newUbrKey.has(key)) {
        try {
          ...
          // User might not exist in new DB
          skipped++;
        }
      }
    }
    console.log(`Inserted ${inserted} user_branch_roles`);
    console.log(`Skipped ${skipped} (user not found)`);
