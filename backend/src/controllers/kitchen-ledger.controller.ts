    ...
    if (updateData.opening_balance !== undefined ||
      ...
      if (current) {
        updateData.closing_balance =
          (updateData.opening_balance ?? current.opening_balance) +
          (updateData.received_quantity ?? current.received_quantity) -
          (updateData.used_quantity ?? current.used_quantity) -
          (updateData.wastage_quantity ?? current.wastage_quantity);
      }
    }
...
export const getKitchenStats = async (req: Request, res: Response) => {
  try {
    ...
    const { data: pendingVariances } = await supabase
      .from('kitchen_variance_logs')
      .select('*', { count: 'exact' })
      .eq('branch_id', branch_id)
    ...
    res.json({
      ...
      data: {
        todayReceipts: receipts?.length || 0,
        pendingVariances: pendingVariances?.length || 0,
