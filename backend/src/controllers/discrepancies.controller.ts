...
export class DiscrepancyController {
  ...
  static async exportAuditReport(req: Request, res: Response) {
    try {
      ...
      const reviewing = records.filter((f: any) => f.status === 'UNDER_REVIEW').length;
      const resolved  = records.filter((f: any) => f.status === 'RESOLVED').length;
      const critical  = records.filter((f: any) => f.severity === 'CRITICAL').length;
      ...
      res.setHeader('Content-Disposition', `attachment; filename="Discrepancy_Audit_Report_${dateStr}.pdf"`);
      doc.pipe(res);
      ...
      const drawHeader = () => {
        ...
        doc.fillColor(D_PRIMARY).fontSize(16).font('Helvetica-Bold')
          .text(COMPANY_NAME, MARGIN + 65, 26, { width: CONTENT_W - 65 });
        doc.fillColor(D_SECONDARY).fontSize(8).font('Helvetica')
          .text(COMPANY_ADDRESS, MARGIN + 65, 46, { width: CONTENT_W - 65 });

        doc.fillColor(D_PRIMARY).fontSize(11).font('Helvetica-Bold')
          .text('DISCREPANCY AUDIT REPORT', MARGIN, 26, { width: CONTENT_W, align: 'right' });
        doc.fillColor(D_SECONDARY).fontSize(8).font('Helvetica')
          .text(`Generated: ${new Date().toLocaleString('en-US')}`, MARGIN, 42, { width: CONTENT_W, align: 'right' });
        const filterLabel = [
          status ? `Status: ${status}` : '',
          severity ? `Severity: ${severity}` : '',
          branch_id ? `Branch ID: ${branch_id}` : ''
        ].filter(Boolean).join('  ·  ') || 'All Flags';
        doc.fillColor(D_SECONDARY).fontSize(8)
          .text(`Filters: ${filterLabel}`, MARGIN, 54, { width: CONTENT_W, align: 'right' });

        doc.strokeColor(D_BORDER).lineWidth(1)
      ...
      const drawSummaryCards = (y: number) => {
        const cards = [
          ...
          { label: 'Critical Flags', value: String(critical),  accent: D_DANGER  },
        ];
        ...
        cards.forEach((c, i) => {
          ...
          doc.fillColor(D_SECONDARY).fontSize(7).font('Helvetica')
            .text(c.label.toUpperCase(), cx + 8, y + 6, { width: cardW - 12 });
          doc.fillColor(D_PRIMARY).fontSize(18).font('Helvetica-Bold')
      ...
      const drawTableHeader = (y: number) => {
        ...
        COLS.forEach(c => doc.text(c.label, c.x, y + 6, { width: c.w, align: 'left' }));
        return y + 20;
      };

      // ── Section title ──────────────────────────────────────────────────────
      const drawSection = (title: string, y: number) => {
        doc.fontSize(11).font('Helvetica-Bold').fillColor(D_PRIMARY)
          .text(title, MARGIN, y);
        doc.strokeColor(D_GOLD).lineWidth(1.5)
          .moveTo(MARGIN, y + 14).lineTo(MARGIN + 200, y + 14).stroke();
        return y + 22;
      };
      ...
      const drawFooter = () => {
        ...
        for (let i = range.start; i < range.start + range.count; i++) {
          doc.switchToPage(i);
          doc.strokeColor(D_BORDER).lineWidth(0.5)
            .moveTo(MARGIN, doc.page.height - 42)
            .lineTo(PAGE_W - MARGIN, doc.page.height - 42).stroke();
          doc.fillColor(D_SECONDARY).fontSize(7.5).font('Helvetica')
            .text(`${COMPANY_NAME}  ·  Discrepancy Audit Report  ·  Confidential`,
              MARGIN, doc.page.height - 32, { width: CONTENT_W * 0.65 });
          doc.fillColor(D_SECONDARY).fontSize(7.5)
            .text(`Page ${i + 1} of ${range.count}`, MARGIN, doc.page.height - 32,
      ...
      records.forEach((flag: any, i: number) => {
        ...
        doc.strokeColor(D_BORDER).lineWidth(0.3)
          .moveTo(MARGIN, y + rowH).lineTo(PAGE_W - MARGIN, y + rowH).stroke();

        doc.fillColor(D_SECONDARY).fontSize(7.5).font('Helvetica')
          .text(String(i + 1), COLS[0].x, y + 6, { width: COLS[0].w });

        doc.fillColor(D_PRIMARY).fontSize(7.5).font('Helvetica')
          .text(flag.branches?.name || '—', COLS[1].x, y + 6, { width: COLS[1].w, ellipsis: true });

        doc.text(flag.record_date ? flag.record_date.split('T')[0] : '—', COLS[2].x, y + 6, { width: COLS[2].w });

        doc.text((flag.flag_type || '').replace(/_/g, ' '), COLS[3].x, y + 6, { width: COLS[3].w, ellipsis: true });

        // Severity badge
        const sevColor = SEV_COLORS[flag.severity] || D_SECONDARY;
        doc.fillColor(sevColor).fontSize(7).font('Helvetica-Bold')
          .text(flag.severity || '—', COLS[4].x, y + 6, { width: COLS[4].w });

        // Status badge
        const stColor = STATUS_COLORS[flag.status] || D_SECONDARY;
        doc.fillColor(stColor).fontSize(7).font('Helvetica-Bold')
          .text((flag.status || '').replace(/_/g, ' '), COLS[5].x, y + 6, { width: COLS[5].w });

        // Description (truncated)
        doc.fillColor(D_SECONDARY).fontSize(7).font('Helvetica')
          .text(flag.description || '—', COLS[6].x, y + 6, { width: COLS[6].w, ellipsis: true });

        // Accountant response on second line if exists
        if (flag.accountant_response) {
          doc.fillColor('#1d4ed8').fontSize(6.5).font('Helvetica')
            .text(`↳ ${flag.accountant_response}`, COLS[1].x, y + 16, { width: CONTENT_W - 20, ellipsis: true });
        }
      ...
      if (records.length === 0) {
        doc.fillColor(D_SECONDARY).fontSize(10).font('Helvetica')
          .text('No discrepancy flags found for the selected filters.', MARGIN, y + 20, { width: CONTENT_W, align: 'center' });
      }
