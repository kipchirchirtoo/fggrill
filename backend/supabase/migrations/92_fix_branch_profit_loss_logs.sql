-- Fix get_branch_profit_loss to use the actual live tables:
--  - cashier_shift_logs (not empty cashier_shifts) for shift data
--  - cashier_shift_transactions joined with cashier_shift_logs for revenue
--  - cashier_shift_logs aggregated totals for verified revenue
--  - branch_payments for expenses
--  - goods_receipt_lines for COGS
...
BEGIN
  ...
  -- (normalizing legacy payment_method spellings defensively)
  SELECT
  ...
  WHERE branch_id = get_branch_profit_loss.branch_id
    AND shift_start::date BETWEEN start_date AND end_date;
