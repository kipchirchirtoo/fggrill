                    ...
                    <div className="p-8 pt-4 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar no-scrollbar">
                        ...
                        <div className="space-y-2">
                            ...
                            <button
                                ...
                                className={`w-full h-12 px-4 bg-stone-50 border border-stone-200 rounded-xl flex items-center justify-between group hover:border-stone-400 transition-all ${!!initialStaffId ? 'opacity-70 cursor-not-allowed' : ''}`}
                            >
                                <div className="flex items-center gap-3 truncate">
                                    <User className={`w-4 h-4 ${formData.staff_id ? 'text-blue-600' : 'text-stone-400'}`} />
                                    <span className={`text-sm tracking-tight truncate ${formData.staff_id ? 'text-stone-900 font-bold' : 'text-stone-400 font-medium'}`}>
                                        {formData.staff_id ? 'Personnel Selected' : 'Search and select staff...'}
                                    </span>
                                </div>
                                <ChevronDown className={`w-4 h-4 ${formData.staff_id ? 'text-blue-600' : 'text-stone-300'} group-hover:text-stone-500 transition-colors`} />
                            </button>
                        </div>
