...
export function CloseShiftModal({
  ...
  const [newCreditBill, setNewCreditBill] = useState({
    staffId: "",
    amount: "",
    name: "",
  });
  ...
  const addCreditBill = () => {
    ...
    setFormData((prev) => ({
      ...
      credit_bills_taken: updatedList.reduce(
  ...
  const removeCreditBill = (id: string) => {
    const updatedList = (formData.credit_bills_details || []).filter(
      (item) => item.id !== id,
    );
  ...
  const removePaidBill = (id: string) => {
    const updatedList = (formData.paid_bills_details || []).filter(
      (item) => item.id !== id,
    );
  ...
  const openingFloat = toAmount(
    currentShift?.opening_float ?? currentShift?.opening_cash_float,
  );
  const cashSales = toAmount(
    currentShift?.total_cash_sales ??
      currentShift?.cash_sales ??
      currentShift?.total_cash_in,
  );
  const mpesaSales = toAmount(
    currentShift?.total_mpesa_sales ??
      currentShift?.mpesa_sales ??
      currentShift?.total_mpesa_in,
  );
  const cardSales = toAmount(
    currentShift?.total_card_sales ??
      currentShift?.card_sales ??
      currentShift?.total_card_in,
  );
  const shiftTotalSales = toAmount(
    currentShift?.total_sales ?? currentShift?.total_revenue,
  );
  const expectedClosing =
    openingFloat + cashSales + (formData.paid_bills_value || 0);
  const variance = (formData.closing_float || 0) - expectedClosing;
  const varianceThreshold = Math.max(expectedClosing * 0.05, 1000);
  const varianceExplanationRequired = Math.abs(variance) > varianceThreshold;

  // Credit bills outstanding
  const outstandingCredit = Math.max(
    0,
    (formData.credit_bills_taken || 0) - (formData.paid_bills_value || 0),
  );

  // Variance label & colour
  const varianceLabel =
    variance === 0 ? "Fully Reconciled" : variance > 0 ? "Surplus" : "Shortage";
  const varianceColor =
    variance === 0
      ? "text-emerald-600"
      : variance > 0
  ...
  const handleSubmit = () => {
    if (varianceExplanationRequired && !formData.notes?.trim()) {
      setSubmissionError(
        "Variance explanation is required before closing this shift. Add the reason in the notes field below.",
      );
      return;
    }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-[92vw] w-[92vw] h-[92vh] flex p-0 overflow-hidden bg-white border border-slate-200 rounded-xl shadow-2xl focus:outline-none focus:ring-0">
        ...
        <div
          className="w-72 bg-slate-50/50 flex flex-col p-5 shrink-0 border-r border-slate-200/60 overflow-y-auto rounded-none"
        ...
        >
          <div className="mb-6">
            ...
            <DialogTitle className="text-lg font-black text-slate-900 tracking-tight mb-1">
          ...
          <div className="flex-1 space-y-3 text-xs">
            ...
            <div className="p-3 bg-white rounded-lg border border-slate-100 shadow-sm space-y-1.5">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-black text-slate-900 tabular-nums">
                  KES {(formData.closing_float || 0).toLocaleString()}
                </span>
              </div>
            </div>
            ...
            <div className="p-3 bg-white rounded-lg border border-slate-100 shadow-sm space-y-1.5">
              ...
              {[
                ...
                >
                  <span className="text-slate-500">{row.label}</span>
                  <span className="font-black text-slate-800 tabular-nums">
                    {row.value > 0 ? `KES ${row.value.toLocaleString()}` : "—"}
                  </span>
                </div>
              ))}
              <div className="border-t border-slate-100 pt-1.5 flex justify-between items-center">
                ...
                <span className="font-black text-slate-900 tabular-nums">
                  KES {shiftTotalSales.toLocaleString()}
                </span>
              </div>
            </div>
            ...
            <div className="p-3 bg-white rounded-lg border border-slate-100 shadow-sm space-y-1.5">
              ...
              {[
                {
                  label: "Restaurant",
                  value: formData.restaurant_revenue || 0,
                },
                { label: "Bar", value: formData.bar_revenue || 0 },
                {
                  label: "Rooms",
                  na: formData.rooms_na,
                  value: formData.room_booking_revenue || 0,
                },
                {
                  label: "Conference",
                  na: formData.conference_na,
                  value: formData.conference_revenue || 0,
                },
                {
                  label: "Pool",
                  na: formData.pool_na,
                  value: formData.swimming_pool_revenue || 0,
                },
                ...
                >
                  ...
                  <span className="font-black text-slate-800 tabular-nums">
                    {row.na ? (
                      <span className="text-slate-300 text-[9px]">N/A</span>
                    ) : row.value > 0 ? (
                      `KES ${row.value.toLocaleString()}`
                    ) : (
                      "—"
            ...
            <div className="p-3 bg-white rounded-lg border border-slate-100 shadow-sm space-y-1.5">
              ...
              {[
                {
                  label: "Created",
                  value: formData.credit_bills_taken || 0,
                  color: "text-orange-600",
                },
                {
                  label: "Paid",
                  value: formData.paid_bills_value || 0,
                ...
                },
                {
                  ...
                  color:
                    outstandingCredit > 0 ? "text-rose-600" : "text-slate-800",
                },
              ].map((row) => (
                <div
                  key={row.label}
            ...
            {/* Variance */}
            <div
              className={`p-3 rounded-lg border transition-all duration-300 ${varianceBg}`}
            >
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2">
                Variance
              </p>
              <p className="text-[9px] text-slate-400 mb-2 leading-relaxed">
                {openingFloat.toLocaleString()} + {cashSales.toLocaleString()} +{" "}
                {(formData.paid_bills_value || 0).toLocaleString()} ={" "}
                {expectedClosing.toLocaleString()}
              </p>
              <div className="flex items-center justify-between">
                <span
                  className={`text-[10px] font-black uppercase tracking-wider ${varianceColor}`}
                >
                  {varianceLabel}
                </span>
                <span
                  className={`text-xl font-black tabular-nums ${varianceColor}`}
                >
                  {formData.closing_float
                    ? variance >= 0
          ...
          <div className="max-w-[1300px] mx-auto space-y-24 pb-32">
              ...
              <div className="flex items-center gap-6 border-b border-slate-100 pb-8">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-widest">
                    Verify physical cash drawer totals
                  </p>
                </div>
              </div>
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="p-8 bg-slate-50/50 rounded-none border border-slate-100 shadow-sm relative overflow-hidden group">
                    ...
                    <p className="text-3xl font-black text-slate-900 tabular-nums">
                      <span className="text-xs font-bold text-slate-300 mr-2 uppercase">
                        KES
                      </span>
                ...
                <div className="p-8 bg-white rounded-none border-2 border-slate-900 shadow-2xl shadow-slate-900/5 relative overflow-hidden transition-all">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <Label className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em]">
                        Actual Cash Count
                      </Label>
                  ...
                  <div className="relative">
                    <span className="absolute left-0 top-1/2 -translate-y-1/2 text-slate-300 font-black text-xl">
                      KES
                    </span>
                    <Input
                      type="number"
            ...
            <section className="space-y-10">
              <div className="flex items-center justify-between border-b border-slate-100 pb-8">
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-widest">
                      Departmental revenue declarations
                    </p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                {[
                  {
                    label: "Swimming Pool",
                    id: "swimming_pool_revenue",
                    icon: Waves,
                    na: "pool_na",
                  },
                  {
                    label: "Pool Tokens",
                    id: "pool_token_revenue",
                    icon: Ticket,
                    na: "pool_tokens_na",
                  },
                  {
                    label: "Conference",
                    id: "conference_revenue",
                    icon: Users,
                    na: "conference_na",
                  },
                  {
                    label: "Rooms",
                    id: "room_booking_revenue",
                    icon: Bed,
                    na: "rooms_na",
                  },
                  {
                    label: "Restaurant",
                    id: "restaurant_revenue",
                  ...
                  {
                    label: "Sports Bar",
                    id: "sports_bar_revenue",
                    icon: Wine,
                    na: "sports_bar_na",
                  },
                  {
                    label: "Executive Bar",
                    id: "executive_bar_revenue",
                    icon: Wine,
                    na: "executive_bar_na",
                  },
                  {
                  ...
                  },
                  {
                    ...
                    id: "cashier_station_revenue",
                    icon: CreditCard,
                    na: "cashier_station_na",
                  },
                ].map((item) => (
                  <div
                    key={item.id}
                    className={`p-6 bg-slate-50/50 rounded-none border border-slate-100 shadow-sm transition-all hover:bg-white hover:shadow-xl hover:-translate-y-0.5 group ${formData[item.na as keyof CloseShiftData] ? "opacity-30 grayscale" : ""}`}
                  >
                    <div className="flex items-center justify-between mb-4">
                      ...
                      {item.na && (
                        <Checkbox
                          ...
                          onCheckedChange={(checked) =>
                            updateField(
                              item.na as keyof CloseShiftData,
                              checked,
                            )
                          }
                    ...
                    <div className="relative">
                      <span className="absolute left-0 top-1/2 -translate-y-1/2 text-[10px] font-black text-slate-300">
                        KES
                      </span>
                      <Input
                        type="number"
                        ...
                        value={
                          (formData[item.na as keyof CloseShiftData]
                            ? ""
                            : (formData[item.id as keyof CloseShiftData] ??
                              "")) as string | number
                        }
                        onChange={(e) =>
                          updateField(
                            item.id as keyof CloseShiftData,
                            parseFloat(e.target.value) || 0,
                          )
                        }
            ...
            <section className="space-y-10">
              <div className="flex items-center gap-6 border-b border-slate-100 pb-8">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-widest">
                    Reconcile staff credit and debt clearance
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="p-8 bg-slate-50/50 rounded-none border border-slate-100">
                  <div className="flex items-center justify-between mb-8">
                    ...
                    <p className="text-2xl font-black text-slate-900 tabular-nums">
                      KES {formData.credit_bills_taken?.toLocaleString()}
                    </p>
                  </div>
                  <div className="grid grid-cols-2 gap-2 mb-8">
                    ...
                    >
                      <span
                        className={`text-[10px] font-black uppercase tracking-widest ${newCreditBill.staffId ? "text-slate-900" : "text-slate-400"}`}
                      >
                    ...
                    <div className="flex gap-2">
                      ...
                      <IOSButton
                        ...
                        className="bg-slate-900 hover:bg-black text-white h-12 w-12 rounded-xl flex items-center justify-center shrink-0 shadow-lg shadow-slate-200"
                        disabled={
                          !newCreditBill.staffId || !newCreditBill.amount
                        }
                      >
                  ...
                  <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                    {(formData.credit_bills_details || []).map((bill: any) => (
                      <div
                        key={bill.id}
                        className="flex items-center justify-between p-4 bg-white rounded-none border border-slate-50 shadow-sm group"
                      >
                        <div className="flex items-center gap-3">
                ...
                <div className="p-8 bg-slate-50/50 rounded-none border border-slate-100">
                  <div className="flex items-center justify-between mb-8">
                    ...
                    <p className="text-2xl font-black text-slate-900 tabular-nums">
                      KES {formData.paid_bills_value?.toLocaleString()}
                    </p>
                  </div>
                  <div className="grid grid-cols-2 gap-2 mb-8">
                    <button
                    ...
                    >
                      <span
                        className={`text-[10px] font-black uppercase tracking-widest ${newPaidBill.staffId ? "text-slate-900" : "text-slate-400"}`}
                      >
                      ...
                      >
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <button
                            ...
                            className="p-2 text-slate-200 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-all opacity-0 group-hover:opacity-100"
                          >
                            <Trash2 className="h-4 w-4" />
                          </button>
                        </div>
            ...
            <section className="space-y-10 pt-16 border-t border-slate-100">
              <div className="flex items-center gap-6">
                ...
                <div>
                  <h3 className="text-xl font-black text-slate-900 tracking-tight uppercase">
                    Handover / Variance Notes
                  </h3>
                  <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-widest">
                    Required when the cash variance exceeds KES{" "}
                    {varianceThreshold.toLocaleString()}
                  </p>
                </div>
              </div>
                ...
                <div className="flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4 text-amber-900">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs font-semibold leading-relaxed">
                      Current variance is KES{" "}
                      {Math.abs(variance).toLocaleString()}. Add the reason
                      before archiving this shift.
                    </p>
                  </div>
                </div>
              )}
              <Textarea
                placeholder={
                  varianceExplanationRequired
                    ...
                    : "Any observations or handover notes..."
                }
