...
interface ShiftLog {
  ...
  status:
    ...
    | "verified"
    | "approved";
  module?: "standard" | "kyogong";
}

export function CashierLogbook({
  type,
  source,
  pointCode,
}: {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        if (!targetCode) {
          ...
          else if (user?.role === UserRole.KYOGONG_EXECUTIVE_BAR_CASHIER)
            targetCode = "EXEC_BAR";
          else if (user?.role === UserRole.KYOGONG_SPORTS_BAR_CASHIER)
            targetCode = "SPORTS_BAR";
          else if (user?.role === UserRole.KYOGONG_RECEPTION_CASHIER)
            targetCode = "RECEPTION";
        }
  ...
  const fetchShifts = async () => {
    try {
      ...
      const [stdRes, kyoRes] = await Promise.all([
        fetchAPI<any[]>(`/cashier/shifts${buildQuery(params)}`),
      ...
      if (kyoRes.success) {
        allShifts = [
          ...allShifts,
          ...(kyoRes.data || []).map((s: any) => ({ ...s, module: "kyogong" })),
        ];
      }
  ...
  const handleStartShift = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        toast.error(
          response.message || response.error || "Failed to start shift",
        );
      }
    } catch (error: any) {
    ...
    try {
      const response = isKyogong
        ? await kyogongAPI.closeShift(currentShift.id, {
            ...
            variance_reason: data.notes?.trim() || undefined,
          })
      ...
      if (response.success) {
        ...
        toast.error(
          response.message || response.error || "Failed to close shift",
        );
      }
    } catch (error: any) {
      console.error("Failed to close shift:", error);
      toast.error(error.message || error.error || "Failed to close shift");
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-stone-500">
            Track your cashier shifts and reconciliation
          </p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
            ...
            <div>
              ...
              <p className="font-bold text-emerald-600">
                KES {currentShift.total_sales.toLocaleString()}
              </p>
            </div>
            <div>
              <p className="text-xs text-stone-500 font-bold uppercase">
          ...
          <div className="text-center py-8">
            ...
            <div className="max-w-sm mx-auto space-y-3">
                  ...
                  >
                    ...
                    {salesPoints.map((p) => (
                      <option key={p.id} value={p.id}>
                        {p.name}{" "}
                        {p.is_occupied
                          ? `(OCCUPIED by ${p.current_shift?.cashier_id === user?.id ? "YOU" : "Another Cashier"})`
                          : ""}
                      </option>
                    ))}
                  </select>
              ...
              <IOSButton
                ...
                disabled={
                  isLoading || !openingFloat || (isKyogong && !selectedPointId)
                }
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <span
                            className={`px-2 py-0.5 text-[10px] font-black rounded-full border ${shift.module === "kyogong" ? "bg-indigo-50 text-indigo-700 border-indigo-200" : "bg-stone-100 text-stone-600 border-stone-200"}`}
                          >
                            {shift.module === "kyogong"
                              ? "KYOGONG"
                              : "FAMOUS GATE"}
                          </span>
                          <span
                            className={`px-2 py-0.5 text-xs font-bold rounded-full border ${getStatusColor(shift.status)}`}
                          >
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 text-sm">
                          ...
                          {shift.variance !== undefined && (
                            <div>
                              ...
                              <p
                                className={`font-bold ${variance === 0 ? "text-emerald-600" : variance > 0 ? "text-blue-600" : "text-rose-600"}`}
                              >
                                {variance > 0 ? "+" : ""}
