...
export function AuthProvider({ children }: { children: ReactNode }) {
      ...
      if (token) {
        ...
        try {
          ...
          if (is401) {
            ...
            console.warn(
              "[Auth V3] getMe() failed (non-auth), keeping cached session:",
              msg,
            );
          }
        }
  ...
  const posLogin = useCallback(
    async (pin: string, redirectTo?: string): Promise<void> => {
      if (isAuthenticating) return;
      setIsAuthenticating(true);
  ...
  const redirectToDashboard = (role: UserRole, isCentral?: boolean) => {
    const roleRedirects: Record<UserRole, string> = {
      ...
      [UserRole.RESTAURANT_CASHIER]: "/dashboard/cashier",
      [UserRole.MAIN_BAR_CASHIER]: "/dashboard/cashier",
      [UserRole.EXECUTIVE_BAR_CASHIER]: "/dashboard/cashier",
      [UserRole.NON_CONSUMABLES_CASHIER]: "/dashboard/cashier",
      ...
      [UserRole.KYOGONG_EXECUTIVE_BAR_CASHIER]:
        "/dashboard/kyogong/executive-bar",
