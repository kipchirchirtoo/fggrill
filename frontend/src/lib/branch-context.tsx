...
export function BranchProvider({ children }: { children: ReactNode }) {
  ...
  const fetchBranches = async () => {
    ...
    setIsLoading(true);
    try {
      // Check if we have branches stored in localStorage first (from demo login)
      const cachedBranches = localStorage.getItem('branches');
      const cachedUserBranches = localStorage.getItem('userBranches');
      
      // If we have cached branches and this is a demo login (token starts with 'demo-token')
      const token = localStorage.getItem('token');
      const isDemo = token?.startsWith('demo-token');
      
      if (isDemo && cachedBranches && cachedUserBranches) {
        console.log('Using cached branch data for demo user');
        try {
      ...
      if (branchesResponse.ok) {
        ...
        // Fetch user's branch access
        const accessResponse = await fetch(`${API_URL}/api/user/branch-access`, {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
        ...
        if (accessResponse.ok) {
          ...
          const accessibleBranchIds = new Set(accessData.data.map((a: any) => a.branch_id));
          
          // Filter branches user has access to
          let accessibleBranches = allBranches.filter(
            (branch: Branch) => accessibleBranchIds.has(branch.id)
          );
          
          // Super admins and general managers have access to all branches
          if (['super_admin', 'general_manager', 'central_operations_manager'].includes(user.role)) {
            accessibleBranches = allBranches;
          }
          
          setUserBranches(accessibleBranches);
          
          // Store user branches for potential offline/demo use
          localStorage.setItem('userBranches', JSON.stringify(accessibleBranches));
        } else {
          // If API call fails, use branch_id from user context in dev mode
          if (process.env.NODE_ENV === 'development') {
            const devUserBranches = allBranches.filter(
              (branch: Branch) => branch.id === user.branch_id || user.is_central
            );
            const userBranches = devUserBranches.length > 0 ? devUserBranches : allBranches;
...
export function BranchSelector() {
  ...
  // Always show the selector if we have branches
  if (branchesToShow.length === 0 || !user) {
    return null;
  }
