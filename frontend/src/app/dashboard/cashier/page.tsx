...
function CashierPageContent() {
    ...
    const [paymentFlowChoice, setPaymentFlowChoice] = useState<'mpesa' | 'cash' | 'card' | null>(null);
    ...
    useEffect(() => {
        if (isPOSType && paymentMethod === 'mpesa' && cart.length > 0) {
            const total = cart.reduce((sum, i) => sum + i.line_total, 0);
            if (total > 0) {
                const timer = setTimeout(() => handleAutoMpesaSearch(total), 1500);
                return () => clearTimeout(timer);
            }
        }
    }, [cart, paymentMethod, isPOSType]);
    ...
    const handleAutoMpesaSearch = async (amount: number) => {
        try {
            ...
            if (response.success && response.data.length > 0) {
                setMpesaTransactionDetails(response.data[0]);
                toast.success('Found matching M-Pesa payment!');
            }
        } catch (error) {
        ...
        try {
            ...
            const receiptData = {
                ...
                items: items.map((item: any) => ({
                    ...
                    total: item.line_total || (item.total) || ((item.unit_price || item.price || 0) * (item.qty || item.quantity || 1))
                })),
                subtotal: subtotal,
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in duration-500">
                                    ...
                                    <div className="lg:col-span-2 space-y-6">
                                        ...
                                        {!isPOSType && (
                                            <div className="flex items-center gap-4 mb-2">
                                                <button
                                                    onClick={() => { setPaymentFlowChoice(null); setBillData(null); setCashGiven(''); }}
                                                ...
                                                <span className="text-xs font-black uppercase tracking-widest text-stone-400">
                                                    Current Flow: {paymentFlowChoice}
                                                </span>
                                            </div>
                                        )}

                                        {/* Search Section */}
                                        <IOSCard className="p-6 border-none shadow-sm bg-white overflow-hidden relative">
                                            {paymentFlowChoice === 'mpesa' && <div className="absolute top-0 right-0 w-32 h-32 bg-emerald-50 -mr-16 -mt-16 rounded-full opacity-50" />}
                                            {paymentFlowChoice === 'cash' && <div className="absolute top-0 right-0 w-32 h-32 bg-stone-100 -mr-16 -mt-16 rounded-full opacity-50" />}

                                            <h2 className="text-lg font-bold mb-4 text-stone-800 flex items-center gap-2 relative">
                                                <Search size={20} className="text-orange-600" />
                                                {isPOSType ? 'Add to Cart' : `Lookup Bill for ${paymentFlowChoice?.toUpperCase()} Payment`}
                                            </h2>

                                            <form onSubmit={isPOSType ? handlePOSSearch : handleScan} className="flex gap-3 relative">
                                                ...
                                                <IOSButton
                                                    type="submit"
                                                    disabled={isLoading}
                                                    className="bg-stone-900 hover:bg-black text-white h-11 px-8"
                                                >
                                                ...
                                                <div className="space-y-4">
                                                    <div className="flex gap-2 p-1 bg-stone-100 rounded-xl">
                                                        <button
                                                            ...
                                                            className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${paymentMethod === 'cash' ? 'bg-white text-stone-900 shadow-sm' : 'text-stone-500'}`}
                                                        >
                                                            Cash
                                                        </button>
                                                        <button
                                                            onClick={() => setPaymentMethod('mpesa')}
                                                            className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${paymentMethod === 'mpesa' ? 'bg-emerald-600 text-white shadow-sm' : 'text-stone-500'}`}
                                                        >
                                                            M-Pesa
                                                        </button>
                                                    </div>
                                                    ...
                                                    {paymentMethod === 'cash' && (
                                                        <div className="space-y-3 animate-in slide-in-from-top-2">
                                                            <div>
                                                                <label className="text-[10px] font-bold text-stone-400 uppercase mb-1 block">Money Received</label>
                                                                <Input
                                                                    type="number"
                                                                    value={cashGiven}
                                                            ...
                                                            {parseFloat(cashGiven) > 0 && (
                                                                <div className="p-3 bg-stone-50 rounded-xl border border-stone-100">
                                                                    <div className="flex justify-between items-center">
                                                                        <span className="text-[10px] font-bold text-stone-500 uppercase">Change Due</span>
                                                                        <span className={`text-lg font-black ${changeDue < 0 ? 'text-rose-500' : 'text-stone-900'}`}>
                                                    ...
                                                    {paymentMethod === 'mpesa' && mpesaTransactionDetails && (
                                                        <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center justify-between animate-in fade-in">
                                                            <div>
                                                                <p className="text-[10px] font-bold text-emerald-800 uppercase">Found Match</p>
                                                                <p className="text-xs font-bold text-stone-900">{mpesaTransactionDetails.receipt}</p>
                                                                <p className="text-[10px] text-stone-500">{mpesaTransactionDetails.name}</p>
                                                            </div>
                                                    ...
                                                    <IOSButton
                                                        ...
                                                        disabled={isProcessing || cart.length === 0 || (paymentMethod === 'cash' && changeDue < 0)}
                                                        className={`w-full h-12 text-lg ${paymentMethod === 'mpesa' ? 'bg-emerald-600' : 'bg-stone-900'}`}
                                                    >
                                                        {isProcessing ? <Loader2 className="animate-spin" /> :
                                                            (paymentMethod === 'mpesa' && mpesaTransactionDetails ? 'Confirm M-Pesa Match' :
                                                                (paymentMethod === 'mpesa' ? 'Send STK Prompt' : 'Complete Cash Sale'))}
                                                    </IOSButton>
                                                </div>
                                            </IOSCard>
                                        )}
                                    ...
                                    <div className="space-y-6">
                                        {billData && billData.financials.balance > 0 && (
                                            <IOSCard className="p-6 border-none shadow-sm bg-white animate-in slide-in-from-right-4">
                                                <h2 className="text-lg font-bold mb-4 text-stone-800 flex items-center gap-2">
                                                    <Calculator size={20} className="text-orange-600" />
                                                    Process {paymentFlowChoice?.toUpperCase()} Payment
                                                </h2>

                                                {paymentFlowChoice === 'cash' ? (
                                                    <div className="space-y-4">
                                                        <div>
                                                        ...
                                                        <div>
                                                            <label className="text-[10px] font-bold text-stone-400 uppercase mb-1 block">Money Received (KES)</label>
                                                            <Input
                                                                type="number"
                                                                value={cashGiven}
                                                                onChange={(e) => {
                                                                    setCashGiven(e.target.value);
                                                                    setPaymentAmount(billData.financials.balance.toString());
                                                                }}
                                                        ...
                                                        {parseFloat(cashGiven) > 0 && (
                                                            <div className="p-4 bg-orange-50 border border-orange-100 rounded-2xl animate-in zoom-in-95">
                                                                <p className="text-[10px] font-bold text-orange-600 uppercase mb-1">Change to be Given</p>
                                                                <p className={`text-2xl font-black ${changeDue < 0 ? 'text-rose-500' : 'text-stone-900'}`}>
                                                                    KES {changeDue.toLocaleString()}
                                                                </p>
                                                                {changeDue < 0 && <p className="text-[10px] text-rose-500 mt-1 font-bold">Insufficient funds</p>}
                                                            </div>
                                                        )}
                                                        <IOSButton
                                                    ...
                                                    <div className="space-y-4">
                                                        <div className="flex gap-2">
                                                            ...
                                                            <IOSButton
                                                                ...
                                                                className="bg-emerald-600 whitespace-nowrap"
                                                            >
                                                                Send STK
                                                            </IOSButton>
                                                        </div>
                                                        ...
                                                        <div className="relative">
                                                            <div className="absolute inset-x-0 top-1/2 h-px bg-stone-100" />
                                                            <span className="relative bg-white px-3 text-[10px] font-bold text-stone-300 uppercase mx-auto block w-fit">OR</span>
                                                        </div>
                                                        ...
                                                        <div className="space-y-2">
                                                            ...
                                                            {mpesaTransactionDetails ? (
                                                                <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-2xl flex items-center justify-between group">
                                                                    <div>
                                                                        <p className="text-sm font-black text-stone-900">{mpesaTransactionDetails.receipt}</p>
                                                                        <p className="text-xs text-stone-500">{mpesaTransactionDetails.name}</p>
                                                                    </div>
                                                                    <button
                                                                        onClick={() => {
                                                                            setMpesaCode(mpesaTransactionDetails.receipt);
                                                                        ...
                                                                        className="px-4 py-2 bg-emerald-600 text-white text-xs font-bold rounded-xl hover:bg-emerald-700 transition-all shadow-md group-hover:scale-105"
                                                                    >
                                                                        Confirm Match
                                                                    </button>
                                                                </div>
                                                                ...
                                                                <div className="text-center p-8 bg-stone-50 rounded-2xl border border-dashed border-stone-200">
                                                                    <RefreshCcw size={20} className="mx-auto text-stone-300 mb-2 animate-spin-slow" />
                                                                    <p className="text-xs text-stone-400">Waiting for payment...</p>
                                                                </div>
                                                            )}
                                                            <button
                                                                onClick={() => handleAutoMpesaSearch(billData.financials.balance)}
                                                                className="w-full text-[10px] font-bold text-orange-600 hover:text-orange-700 uppercase pt-2 flex items-center justify-center gap-1"
                                                            >
                                                                <RefreshCcw size={10} /> Refresh Transactions
                                                            </button>
                                                            <button
                                                                onClick={handleSearchPayment}
                                                                className="w-full text-[10px] font-bold text-stone-400 hover:text-stone-600 uppercase pt-1 flex items-center justify-center gap-1"
                                                            >
                                                                <Search size={10} /> Manual Search
                                                            </button>
                                                        </div>
                                                    </div>
                                                ) : (
                                        ...
                                        <IOSCard className="p-6 border-none shadow-sm bg-white">
                                            <div className="flex items-center justify-between mb-4">
                                                ...
                                                <span className="text-[10px] font-black text-stone-400 uppercase tracking-widest">{transactionHistory.length} TXNS</span>
                                            </div>
                                            {transactionHistory.length === 0 ? (
                                                <div className="text-center py-12">
                                                    <Wallet size={32} className="mx-auto text-stone-200 mb-2" />
                                                    <p className="text-stone-400 text-sm italic">No transactions today</p>
                                                </div>
                                            ) : (
