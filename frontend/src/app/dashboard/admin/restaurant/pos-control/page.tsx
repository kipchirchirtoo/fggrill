      ...
      <DashboardLayout>
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <IOSCard className="p-4"><Power className="h-6 w-6 text-[#34C759] mb-2" /><p className="text-sm text-gray-500">Online</p><p className="text-xl font-bold text-[#34C759]">{stats.online}</p></IOSCard>
            <IOSCard className="p-4"><AlertTriangle className="h-6 w-6 text-[#FF3B30] mb-2" /><p className="text-sm text-gray-500">Offline</p><p className="text-xl font-bold text-[#FF3B30]">{stats.offline}</p></IOSCard>
          </div>
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div className={`p-3 rounded-ios-lg ${terminal.status === 'online' ? 'bg-green-100' : 'bg-red-100'}`}>
                        <Monitor className={`h-6 w-6 ${terminal.status === 'online' ? 'text-[#34C759]' : 'text-[#FF3B30]'}`} />
                      </div>
                      <div>
