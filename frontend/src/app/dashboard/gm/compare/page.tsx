                    ...
                    <div key={branch.branch_id} className="space-y-2">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <span className="w-6 h-6 rounded-full bg-blue-100 flex items-center justify-center text-sm font-bold text-[#007AFF]">{i + 1}</span>
                      ...
                      <div className="h-3 bg-gray-200 rounded-full overflow-hidden">
                        <div 
                          className={`h-full rounded-full ${i === 0 ? 'bg-[#34C759]' : i === 1 ? 'bg-[#007AFF]' : 'bg-gray-400'}`}
            ...
            {branches.map((branch) => (
              <IOSCard key={branch.branch_id} className="p-4">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  ...
                  <div className="p-2 bg-gray-50 rounded"><p className="text-gray-500">Profit</p><p className={`font-bold ${branch.profit >= 0 ? 'text-[#34C759]' : 'text-[#FF3B30]'}`}>KES {branch.profit?.toLocaleString()}</p></div>
