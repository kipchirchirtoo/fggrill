...
export default function BranchManagerDashboard() {
  ...
  const currentBranchIdString: string | null = currentBranchId
  ...
  const fetchData = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const [financeRes, bookingsRes, staffRes, tasksRes, notificationsRes] =
      ...
      setStats({
        ...
        pendingTasks:
          tasksRes.status === "fulfilled"
            ? (tasksRes.value?.data || []).filter(
                (t: any) => t.status === "pending",
              ).length
            : 0,
        arrivals: bookings.filter((b: any) => b.check_in?.startsWith(today))
          .length,
        departures: bookings.filter((b: any) => b.check_out?.startsWith(today))
          .length,
        totalRooms: 0,
    ...
    >
      <DashboardLayout>
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-stone-500 mt-0.5">
                {activeBranch?.name || user?.branch_name || "Your Branch"}
              </p>
            </div>
          ...
          {currentBranchIdString && (
            <PerformanceSummaryWidget branchId={currentBranchIdString} />
          )}
          ...
          {/* Alerts Widget */}
          {currentBranchIdString && (
            <AlertsWidget branchId={currentBranchIdString} />
          )}
              ...
              {quickLinks.map((link) => (
                <Link key={link.href} href={link.href}>
                  <div className="action-card group py-3">
                    ...
                    <p className="action-card-label text-[12px]">
                      {link.label}
                    </p>
                  </div>
                </Link>
            ...
            <div className="lg:col-span-2 card-elevated p-5">
              ...
              <div className="grid sm:grid-cols-3 gap-4">
                <div className="p-4 bg-stone-50 rounded-lg border border-stone-100">
                  ...
                  <p className="text-[28px] font-semibold text-stone-900">
                    {stats.arrivals}
                  </p>
              ...
              {alerts.length === 0 ? (
                ...
                <div className="space-y-2">
                  {alerts.map((alert, i) => (
                    <div
                      key={i}
                      className="p-3 rounded-lg bg-stone-50 hover:bg-stone-100 transition-colors cursor-pointer"
                    >
          ...
          <div className="card-elevated p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/dashboard/branch-manager/reservations">
                <button className="text-[13px] font-medium text-stone-600 hover:text-stone-900">
                  View All
                </button>
              </Link>
            </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-stone-100">
                      <th className="text-left py-2 px-3 text-[11px] font-medium text-stone-500 uppercase">
                        Guest
                      </th>
                      <th className="text-left py-2 px-3 text-[11px] font-medium text-stone-500 uppercase">
                        Room
                      ...
                      <th className="text-right py-2 px-3 text-[11px] font-medium text-stone-500 uppercase">
                        Actions
                      </th>
                    </tr>
                  </thead>
                    ...
                    {recentBookings.map((booking: any, i: number) => (
                      ...
                      >
                        ...
                        <td className="py-3 px-3 text-right">
                          <Link
                            href={`/dashboard/branch-manager/reservations/${booking.id}`}
                          >
