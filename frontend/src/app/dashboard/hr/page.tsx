...
export default function HRDashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [staffRes, payrollRes, leaveRes, attendanceRes] =
      ...
      setStats({
        totalStaff:
          staffRes.status === "fulfilled"
            ? staffRes.value?.data?.length || 0
            : 0,
        totalPayroll:
          payrollRes.status === "fulfilled"
            ? payrollRes.value?.data?.totalGrossPay || 0
            : 0,
        pendingLeave:
          leaveRes.status === "fulfilled"
            ? leaveRes.value?.data?.length || 0
            : 0,
        presentToday:
          attendanceRes.status === "fulfilled"
            ? attendanceRes.value?.data?.length || 0
            : 0,
      });
    } catch (error) {
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-stone-500">
                Personnel management and payroll oversight
              </p>
              {activeBranch && (
            ...
            <div className="flex items-center gap-3">
              {userBranches.length > 1 && (
                <BranchSelector showLabel={false} className="min-w-[200px]" />
              )}
              <button
                onClick={fetchData}
                disabled={isLoading}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="stat-card border-l-4 border-l-stone-900">
              <div className="flex justify-between items-start">
                <div>
                  <p className="stat-label uppercase tracking-widest text-[10px]">
                    Total Workforce
                  </p>
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="stat-value text-2xl">
                    KES {stats.totalPayroll.toLocaleString()}
                  </p>
                </div>
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            <div className="card-elevated p-6 border-l-4 border-l-amber-300">
              <div className="flex items-center justify-between mb-6">
                ...
                <Link
                  href="/dashboard/hr/leave"
                  className="text-[12px] text-amber-600 font-bold hover:underline"
                >
                  View Repository
                </Link>
              </div>
              ...
              {stats.pendingLeave === 0 ? (
                ...
                <div className="space-y-4">
                  <div className="p-4 bg-stone-50 rounded-xl border border-stone-100">
                    <p className="text-[13px] font-bold text-stone-900 mb-1">
                      Attention Required
                    </p>
                    <p className="text-[12px] text-stone-500 leading-relaxed">
                      There are currently {stats.pendingLeave} leave
                      application(s) awaiting administrative decision.
                    </p>
                  </div>
                  <Link
                    href="/dashboard/hr/leave"
                    className="block text-center py-2.5 bg-stone-900 text-white rounded-lg text-[13px] font-bold active:scale-95 transition-all"
                  >
              ...
              <div className="space-y-4">
                <div className="p-4 bg-stone-50 rounded-xl border border-stone-100">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-[13px] font-bold text-stone-600 uppercase tracking-tighter">
                      Current Period
                    </span>
                ...
                <Link
                  href="/dashboard/hr/payroll"
                  className="flex items-center justify-between p-4 rounded-xl border border-stone-50 hover:bg-stone-50 hover:border-stone-200 transition-all group"
                >
                  <div className="flex items-center gap-3">
...
function DirectorTasksHRWidget() {
  ...
  useEffect(() => {
    ...
    fetch(
      `${apiBase}/api/finance/director/tasks?status=PENDING&assigned_to_role=hr_manager`,
      {
    ...
    <div className="bg-white border-2 border-rose-200 rounded-2xl shadow-sm overflow-hidden">
      ...
      <div className="divide-y divide-stone-100">
        {tasks.map((task) => (
          <div
            key={task.id}
            className={`p-4 border-l-4 ${PRIO[task.priority] || "border-stone-200"}`}
          >
            <div className="flex justify-between items-start gap-4">
              ...
              <button
                ...
                className="px-3 py-1.5 bg-stone-900 text-white rounded-lg text-xs font-bold hover:bg-stone-700 whitespace-nowrap"
              >
                Respond
              </button>
            </div>
            {responding === task.id && (
              <div className="mt-3 space-y-2">
                <textarea
                  ...
                  className="w-full border border-stone-200 rounded-xl px-3 py-2 text-sm outline-none resize-none"
                />
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="px-4 border border-stone-200 rounded-lg text-xs font-bold text-stone-600"
                  >
                    Cancel
                  </button>
                </div>
