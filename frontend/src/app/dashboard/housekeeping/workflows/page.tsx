        ...
        <div className="space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {workflows.map((workflow) => (
              <IOSCard 
                key={workflow.id} 
                className={`p-6 cursor-pointer transition hover:shadow-none 0_2px_14px_rgba(0,0,0,0.06)] ${selectedWorkflow?.id === workflow.id ? 'ring-2 ring-blue-500' : ''}`}
                    ...
                    <div key={step.id} className="flex items-center gap-4">
                      <div className="flex flex-col items-center">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center ${step.required ? 'bg-blue-100' : 'bg-gray-100'}`}>
                          <Icon className={`h-5 w-5 ${step.required ? 'text-[#007AFF]' : 'text-gray-500'}`} />
                        </div>
