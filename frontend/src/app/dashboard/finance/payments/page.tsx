      ...
      <DashboardLayout>
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            <IOSCard className="p-4"><p className="text-sm text-gray-500">Net Balance</p><p className={`text-2xl font-bold ${stats.income - stats.expense >= 0 ? 'text-[#34C759]' : 'text-[#FF3B30]'}`}>KES {(stats.income - stats.expense).toLocaleString()}</p></IOSCard>
            <IOSCard className="p-4 border-l-4 border-green-500"><p className="text-sm text-gray-500">Total Income</p><p className="text-2xl font-bold text-[#34C759]">KES {stats.income.toLocaleString()}</p></IOSCard>
            <IOSCard className="p-4 border-l-4 border-red-500"><p className="text-sm text-gray-500">Total Expenses</p><p className="text-2xl font-bold text-[#FF3B30]">KES {stats.expense.toLocaleString()}</p></IOSCard>
          </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <div className={`w-12 h-12 rounded-ios-lg flex items-center justify-center ${tx.type === 'income' ? 'bg-green-100' : 'bg-red-100'}`}>
                        {tx.type === 'income' ? <ArrowDownRight className="h-6 w-6 text-[#34C759]" /> : <ArrowUpRight className="h-6 w-6 text-[#FF3B30]" />}
                      </div>
                      <div>
                    ...
                    <p className={`font-bold text-lg ${tx.type === 'income' ? 'text-[#34C759]' : 'text-[#FF3B30]'}`}>
