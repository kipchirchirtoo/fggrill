...
export default function AuditorDashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (logsRes.success) {
        ...
        const pendingRequests = requestsRes.success
          ? (requestsRes.data || []).length
          : 0;
        const voidedCount = ordersRes.success
        ...
        const score =
          logs.length > 0
            ? Math.max(
                ...
                100 -
                  highRisks * 10 -
                  logs.filter((l: any) => l.severity === "medium").length * 2,
              )
            : 100;
  ...
  useKeyboardShortcut(
    "ctrl+e",
    () => router.push("/dashboard/auditor/audit-reports"),
    { enabled: true },
  );
  useKeyboardShortcut("/", () => router.push("/dashboard/auditor/search"), {
    enabled: true,
  });
  useKeyboardShortcut(
    "ctrl+f",
        ...
        <div className="space-y-8 pb-12">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="stat-card">
              ...
              <p
                className={`stat-value ${stats.highRiskFindings > 0 ? "text-rose-600" : ""}`}
              >
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="section-header">
                <div>
                  ...
                  <p className="section-subtitle">
                    System verification and oversight modules
                  </p>
                </div>
              </div>
              ...
              <div className="section-header pt-6">
                <div>
                  ...
                  <p className="section-subtitle">
                    Staff management and payroll auditing
                  </p>
                </div>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                ...
                {watchList.length > 0 ? (
                  watchList
                    ...
                    .map((item: any, i: number) => (
                      <div
                        key={i}
                        className="flex gap-4 items-start group pb-4 border-b border-stone-50 last:border-0 last:pb-0 cursor-pointer"
                        onClick={() => {
                          ...
                          if (item.id && uuidRegex.test(item.id)) {
                            router.push(
                              `/dashboard/auditor/revenue-oversight/details/${item.id}?type=${item.entity_type}`,
                            );
                          } else {
                  ...
                  <div className="empty-state py-8">
                    ...
                    <p className="empty-state-description">
                      The system is currently operating within normal
                      parameters.
                    </p>
                  </div>
                )}
...
function DirectorTasksAuditorWidget() {
  ...
  React.useEffect(() => {
    ...
    fetch(
      `${apiBase}/api/finance/director/tasks?status=PENDING&assigned_to_role=auditor`,
      {
