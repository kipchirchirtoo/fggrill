...
export default function AuditDashboard() {
  ...
  const quickLinks = [
    { href: '/dashboard/audit/logs', icon: ClipboardList, label: 'Audit Logs', desc: 'View all logs', color: 'bg-blue-50 text-blue-600' },
    { href: '/dashboard/audit/reports', icon: FileText, label: 'Reports', desc: 'Audit reports', color: 'bg-green-50 text-green-600' },
      ...
      <DashboardLayout>
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <IOSCard className="p-4"><ClipboardList className="h-8 w-8 text-blue-600 mb-2" /><p className="text-sm text-gray-500">Total Audits</p><p className="text-2xl font-bold">{stats.totalAudits}</p></IOSCard>
            ...
            <IOSCard className="p-4"><CheckCircle className="h-8 w-8 text-green-600 mb-2" /><p className="text-sm text-gray-500">Compliance Score</p><p className="text-2xl font-bold text-green-600">{stats.complianceScore}%</p></IOSCard>
            <IOSCard className="p-4"><AlertTriangle className="h-8 w-8 text-red-600 mb-2" /><p className="text-sm text-gray-500">High Findings</p><p className="text-2xl font-bold text-red-600">{stats.recentFindings}</p></IOSCard>
          </div>
          ...
          <IOSCard className="p-6">
            ...
            <div className="h-4 bg-gray-200 rounded-full overflow-hidden">
              <div className="h-full bg-green-500" style={{ width: `${stats.complianceScore}%` }} />
            </div>
