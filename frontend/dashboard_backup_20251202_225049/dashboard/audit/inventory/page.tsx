      ...
      <DashboardLayout>
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <IOSCard className="p-4"><Package className="h-6 w-6 text-blue-600 mb-2" /><p className="text-sm text-gray-500">Total Items</p><p className="text-xl font-bold">{stats.total}</p></IOSCard>
            <IOSCard className="p-4 border-l-4 border-green-500"><CheckCircle className="h-6 w-6 text-green-600 mb-2" /><p className="text-sm text-gray-500">Matches</p><p className="text-xl font-bold text-green-600">{stats.matches}</p></IOSCard>
            <IOSCard className="p-4 border-l-4 border-red-500"><AlertTriangle className="h-6 w-6 text-red-600 mb-2" /><p className="text-sm text-gray-500">Shortages</p><p className="text-xl font-bold text-red-600">{stats.shortages}</p></IOSCard>
            <IOSCard className="p-4 border-l-4 border-blue-500"><BarChart3 className="h-6 w-6 text-blue-600 mb-2" /><p className="text-sm text-gray-500">Total Variance</p><p className="text-xl font-bold">{stats.totalVariance}</p></IOSCard>
          </div>
                  ...
                  {filteredItems.map((item) => {
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-gray-50">
                        ...
                        <td className={`p-3 text-center font-medium ${item.variance < 0 ? 'text-red-600' : item.variance > 0 ? 'text-blue-600' : ''}`}>
