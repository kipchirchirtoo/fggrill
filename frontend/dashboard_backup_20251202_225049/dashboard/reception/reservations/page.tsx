          ...
          {['dates', 'room', 'guest', 'confirm'].map((s, i) => (
            <div key={s} className="flex items-center">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${
                ...
                ['dates', 'room', 'guest', 'confirm'].indexOf(step) > i ? 'bg-green-500 text-white' : 'bg-gray-200'
              }`}>
                {i + 1}
              </div>
              {i < 3 && <div className={`w-16 h-1 mx-2 ${['dates', 'room', 'guest', 'confirm'].indexOf(step) > i ? 'bg-green-500' : 'bg-gray-200'}`} />}
            </div>
          ))}
        </div>
            ...
            <div className="grid grid-cols-2 gap-3 max-h-64 overflow-y-auto">
              {availableRooms.map((room) => (
                ...
                >
                  ...
                  <p className="font-medium text-blue-600">KES {room.price_per_night?.toLocaleString()}/night</p>
                </div>
              ))}
            </div>
            ...
            <div className="p-4 bg-blue-50 rounded-ios-lg">
              <div className="flex justify-between">
                <div>
                  ...
                  <p className="text-sm text-blue-600">{selectedGuest.phone}</p>
                </div>
                <div className="text-right">
                  <p className="font-bold">Room {selectedRoom.room_number}</p>
                  <p className="text-sm text-blue-600 capitalize">{selectedRoom.room_type}</p>
                </div>
              </div>
                      ...
                      return (
                        <tr key={booking.id} className="hover:bg-gray-50">
                          ...
                          <td className="p-4">
                            ...
                            <p className="text-sm text-green-600">Paid: KES {booking.amount_paid?.toLocaleString()}</p>
                          </td>
                          <td className="p-4">
                          ...
                          <td className="p-4">
                            <div className="flex justify-end gap-2">
                              {booking.status === 'pending' && (
                                <IOSButton size="sm" variant="outline" className="text-red-600" onClick={() => handleCancel(booking)}>
