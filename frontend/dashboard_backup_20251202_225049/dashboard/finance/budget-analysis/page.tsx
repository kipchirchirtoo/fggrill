        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <IOSCard className="p-4">
              ...
              <p className={`text-2xl font-bold ${totalActual > totalBudget ? 'text-red-600' : 'text-green-600'}`}>KES {totalActual.toLocaleString()}</p>
            </IOSCard>
            <IOSCard className="p-4">
              <p className="text-sm text-gray-500">Categories Over Budget</p>
              <p className={`text-2xl font-bold ${overBudget > 0 ? 'text-red-600' : 'text-green-600'}`}>{overBudget}</p>
            </IOSCard>
          </div>
                  ...
                  <IOSCard key={i} className={`p-4 ${isOver ? 'border-l-4 border-red-500' : ''}`}>
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-3">
                        {isOver ? <AlertTriangle className="h-5 w-5 text-red-500" /> : <CheckCircle className="h-5 w-5 text-green-500" />}
                    ...
                    <div className="grid grid-cols-3 gap-4 text-sm mb-3">
                      ...
                      <div><p className="text-gray-500">Variance</p><p className={`font-medium ${item.variance < 0 ? 'text-red-600' : 'text-green-600'}`}>{item.variance < 0 ? '' : '+'}KES {item.variance.toLocaleString()}</p></div>
                    </div>
                    <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                      <div className={`h-full ${isOver ? 'bg-red-500' : 'bg-green-500'}`} style={{ width: `${Math.min(item.percentUsed, 100)}%` }} />
                    </div>
                  </IOSCard>
                );
              })}
