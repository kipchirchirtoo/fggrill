...
export default function FinanceDashboard() {
  ...
  const quickLinks = [
    { href: '/dashboard/finance/expenses', icon: Receipt, label: 'Expenses', desc: 'Manage expenses', color: 'bg-red-50 text-red-600' },
    { href: '/dashboard/finance/invoices', icon: FileText, label: 'Invoices', desc: 'View invoices', color: 'bg-blue-50 text-blue-600' },
    { href: '/dashboard/finance/profit-loss', icon: BarChart3, label: 'P&L', desc: 'Profit & Loss', color: 'bg-green-50 text-green-600' },
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <IOSCard className="p-4">
              <div className="flex items-center justify-between">
                ...
                {data.revenueChange >= 0 ? <ArrowUpRight className="h-5 w-5 text-green-500" /> : <ArrowDownRight className="h-5 w-5 text-red-500" />}
              </div>
              ...
              <p className={`text-xs ${data.revenueChange >= 0 ? 'text-green-600' : 'text-red-600'}`}>{data.revenueChange >= 0 ? '+' : ''}{data.revenueChange}% from last month</p>
            </IOSCard>
            <IOSCard className="p-4">
              <div className="flex items-center justify-between">
                <div className="p-2 bg-red-100 rounded-ios-lg"><TrendingDown className="h-5 w-5 text-red-600" /></div>
                {data.expenseChange <= 0 ? <ArrowDownRight className="h-5 w-5 text-green-500" /> : <ArrowUpRight className="h-5 w-5 text-red-500" />}
              </div>
              ...
              <p className={`text-xs ${data.expenseChange <= 0 ? 'text-green-600' : 'text-red-600'}`}>{data.expenseChange >= 0 ? '+' : ''}{data.expenseChange}% from last month</p>
            </IOSCard>
            <IOSCard className="p-4">
              <div className="p-2 bg-blue-100 rounded-ios-lg w-fit"><TrendingUp className="h-5 w-5 text-blue-600" /></div>
              <p className="text-sm text-gray-500 mt-2">Net Profit</p>
              <p className={`text-xl font-bold ${data.netProfit >= 0 ? 'text-green-600' : 'text-red-600'}`}>KES {data.netProfit?.toLocaleString()}</p>
            </IOSCard>
            <IOSCard className="p-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              {quickLinks.map((link) => (
                <Link key={link.href} href={link.href}>
                  <IOSCard className={`p-4 hover:shadow-lg transition cursor-pointer ${link.color.split(' ')[0]}`}>
