          ...
          <div className="grid grid-cols-3 gap-4">
            <IOSCard className="p-4">
              <div className="flex items-center gap-3">
                ...
                <div><p className="text-sm text-gray-500">Receivables</p><p className="text-xl font-bold text-green-600">KES {totalAR.toLocaleString()}</p></div>
              </div>
            </IOSCard>
            <IOSCard className="p-4">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-red-100 rounded-ios-lg"><ArrowUpRight className="h-5 w-5 text-red-600" /></div>
                <div><p className="text-sm text-gray-500">Payables</p><p className="text-xl font-bold text-red-600">KES {totalAP.toLocaleString()}</p></div>
              </div>
            </IOSCard>
            <IOSCard className="p-4">
              <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <div className={`w-10 h-10 rounded-ios-lg flex items-center justify-center ${item.type === 'receivable' ? 'bg-green-100' : 'bg-red-100'}`}>
                        {item.type === 'receivable' ? <ArrowDownRight className="h-5 w-5 text-green-600" /> : <ArrowUpRight className="h-5 w-5 text-red-600" />}
                      </div>
                      <div>
                    ...
                    <div className="flex items-center gap-4">
                      <p className={`font-bold text-lg ${item.type === 'receivable' ? 'text-green-600' : 'text-red-600'}`}>KES {item.amount.toLocaleString()}</p>
