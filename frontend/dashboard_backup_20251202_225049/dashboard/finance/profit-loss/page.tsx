...
export default function ProfitLossPage() {
  ...
  const LineItem = ({ label, amount, indent = false, bold = false, highlight = false }: { label: string; amount: number; indent?: boolean; bold?: boolean; highlight?: boolean }) => (
    <div className={`flex justify-between py-2 ${indent ? 'pl-4' : ''} ${bold ? 'font-bold' : ''} ${highlight ? 'bg-gray-50 -mx-4 px-4' : ''}`}>
      <span>{label}</span>
      <span className={amount < 0 ? 'text-red-600' : ''}>{amount < 0 ? '(' : ''}KES {Math.abs(amount).toLocaleString()}{amount < 0 ? ')' : ''}</span>
    </div>
  );
            ...
            <>
                ...
                <IOSCard className="p-4">
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-ios-lg ${data.netProfit >= 0 ? 'bg-blue-100' : 'bg-red-100'}`}>
                      {data.netProfit >= 0 ? <TrendingUp className="h-5 w-5 text-blue-600" /> : <TrendingDown className="h-5 w-5 text-red-600" />}
                    </div>
                    <div><p className="text-sm text-gray-500">Net Profit</p><p className={`text-xl font-bold ${data.netProfit >= 0 ? 'text-green-600' : 'text-red-600'}`}>KES {data.netProfit.toLocaleString()}</p></div>
                  </div>
                </IOSCard>
                <IOSCard className="p-4">
                  <div className="flex items-center gap-3">
              ...
              <IOSCard className="p-6">
                ...
                <div className="divide-y">
                  ...
                  <div className={`flex justify-between py-3 font-bold text-lg ${data.netProfit >= 0 ? 'text-green-600' : 'text-red-600'} bg-gray-100 -mx-6 px-6 mt-2`}>
