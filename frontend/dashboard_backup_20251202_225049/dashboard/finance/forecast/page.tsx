          ...
          <div className="grid grid-cols-2 gap-4">
            <IOSCard className="p-4">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-green-100 rounded-ios-lg"><ArrowUpRight className="h-5 w-5 text-green-600" /></div>
            ...
            <IOSCard className="p-4">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-blue-100 rounded-ios-lg"><TrendingUp className="h-5 w-5 text-blue-600" /></div>
                <div><p className="text-sm text-gray-500">Projected Profit ({months}mo)</p><p className="text-xl font-bold text-blue-600">KES {totalProjectedProfit.toLocaleString()}</p></div>
              </div>
            </IOSCard>
          </div>
                ...
                {forecasts.map((f, i) => (
                  <div key={i} className="p-4 bg-gray-50 rounded-ios-lg">
                    ...
                    <div className="grid grid-cols-3 gap-4 text-sm">
                      <div><p className="text-gray-500">Revenue</p><p className="font-bold text-green-600">KES {f.projectedRevenue.toLocaleString()}</p></div>
                      <div><p className="text-gray-500">Expenses</p><p className="font-bold text-red-600">KES {f.projectedExpenses.toLocaleString()}</p></div>
                      <div><p className="text-gray-500">Profit</p><p className={`font-bold ${f.projectedProfit >= 0 ? 'text-blue-600' : 'text-red-600'}`}>KES {f.projectedProfit.toLocaleString()}</p></div>
                    </div>
                  </div>
                ))}
              </div>
