              ...
              {branches.map((branch) => (
                <IOSCard key={branch.id} className="p-6">
                  <div className="flex items-start gap-4">
                    <div className="p-3 bg-blue-100 rounded-ios-lg"><Building2 className="h-6 w-6 text-blue-600" /></div>
                    <div className="flex-1">
                      <p className="font-bold text-lg">{branch.name}</p>
                      <p className="text-sm text-gray-500">{branch.location}</p>
                      <div className="grid grid-cols-2 gap-4 mt-4">
                        <div className="flex items-center gap-2"><Package className="h-4 w-4 text-blue-600" /><span className="text-sm">{branch.totalItems || 0} items</span></div>
