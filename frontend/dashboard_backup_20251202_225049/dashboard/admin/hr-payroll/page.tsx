...
import { Users, DollarSign, RefreshCw, Calendar, FileText, CreditCard } from 'lucide-react';
...
export default function AdminHRPayrollPage() {
  ...
  const quickLinks = [
    { href: '/dashboard/admin/staff', icon: Users, label: 'Staff', desc: 'Manage employees', color: 'bg-blue-50 text-blue-600' },
    { href: '/dashboard/admin/staff/attendance', icon: Calendar, label: 'Attendance', desc: 'Daily records', color: 'bg-green-50 text-green-600' },
    { href: '/dashboard/gm/leave-requests', icon: FileText, label: 'Leave', desc: 'Leave requests', color: 'bg-yellow-50 text-yellow-600' },
    { href: '/dashboard/finance/payments', icon: CreditCard, label: 'Payroll', desc: 'Process payments', color: 'bg-purple-50 text-purple-600' },
  ];
      ...
      <DashboardLayout>
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            <IOSCard className="p-4"><Users className="h-6 w-6 text-blue-600 mb-2" /><p className="text-sm text-gray-500">Total Staff</p><p className="text-xl font-bold">{stats.totalStaff}</p></IOSCard>
            <IOSCard className="p-4"><DollarSign className="h-6 w-6 text-green-600 mb-2" /><p className="text-sm text-gray-500">Monthly Payroll</p><p className="text-xl font-bold">KES {stats.totalPayroll.toLocaleString()}</p></IOSCard>
