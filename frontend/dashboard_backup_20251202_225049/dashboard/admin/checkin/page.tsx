    ...
    <ProtectedRoute allowedRoles={[UserRole.SUPER_ADMIN, UserRole.GENERAL_MANAGER, UserRole.RECEPTIONIST]}>
      <DashboardLayout>
        <div className="space-y-6">
          ...
          <IOSCard className="p-4"><UserCheck className="h-6 w-6 text-green-600 mb-2" /><p className="text-sm text-gray-500">Pending Check-ins</p><p className="text-xl font-bold">{arrivals.length}</p></IOSCard>
                ...
                <IOSCard key={arrival.id} className="p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-ios-lg bg-green-100 flex items-center justify-center"><User className="h-6 w-6 text-green-600" /></div>
                      <div>
