...
async function runImport() {
  ...
  try {
    ...
    // Note: room_types might be shared, but since table check didn't show branch_id, I'll update instead of delete if they exist.
    // Actually, I'll just UPSERT them.

    console.log('--- Importing Room Types ---');
    const typeMap = {};
    for (const rt of roomTypesData) {
      const res = await client.query(`
        INSERT INTO room_types (name, base_price, max_occupancy, code, active)
        VALUES ($1, $2, $3, $4, true)
        ON CONFLICT (id) DO UPDATE SET base_price = EXCLUDED.base_price, max_occupancy = EXCLUDED.max_occupancy, code = EXCLUDED.code
        RETURNING id
      `, [rt.name, rt.base_price, rt.max_occupancy, rt.code]);
      
      // Since ON CONFLICT (id) won't work without id, I'll use a better approach: check by name.
    ...
    for (const rData of roomsData) {
      ...
      for (const roomNum of rData.rooms) {
        await client.query(`
          INSERT INTO rooms (room_number, type_id, branch_id, status, floor, active)
    ...
    for (const hall of hallsData) {
      await client.query(`
        INSERT INTO conference_halls (branch_id, name, capacity, base_price_per_day, status)
        VALUES ($1, $2, $3, $4, 'active')
      `, [BRANCH_ID, hall.name, hall.capacity, hall.price]);
    }
