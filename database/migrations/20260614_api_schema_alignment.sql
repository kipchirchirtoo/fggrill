...
-- Fixes all visible backend API errors by adding missing columns
-- and aliasing existing columns to match backend expectations.
-- ============================================================

-- 1. cashier_shifts: backend expects start_time/end_time, DB has opened_at/closed_at
ALTER TABLE public.cashier_shifts
  ADD COLUMN IF NOT EXISTS start_time TIMESTAMPTZ,
  ADD COLUMN IF NOT EXISTS end_time TIMESTAMPTZ,
  ADD COLUMN IF NOT EXISTS cashier_name TEXT;

-- Sync opened_at -> start_time and closed_at -> end_time
...
ALTER TABLE public.staff_profiles
  ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'active' CHECK (status IN ('active', 'inactive', 'terminated', 'suspended', 'on_leave'));

-- Sync employment_status -> status
UPDATE public.staff_profiles SET status = employment_status WHERE status IS NULL AND employment_status IS NOT NULL;

-- 4. payment_verifications: backend expects recorded_at
ALTER TABLE public.payment_verifications
  ADD COLUMN IF NOT EXISTS recorded_at TIMESTAMPTZ DEFAULT NOW();

-- 5. accounting_customers: backend expects customer_name (DB has name)
-- Add generated column or alias via view. For Supabase/PostgREST, add a real column.
...
-- 6. payroll_batches: backend expects batch_number (DB has batch_name)
...
-- 9. Create department_accounts table (missing entirely)
...
-- 10. Add missing FK relationship hint for payments -> reservations
-- Backend joins payments and reservations; add reservation_id to payments if missing
DO \$\$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'payments' AND table_schema = 'public')
     AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'reservations' AND table_schema = 'public')
  THEN
...
-- 11. stock_takes: add missing columns that backend might expect
ALTER TABLE public.stock_takes
  ADD COLUMN IF NOT EXISTS notes TEXT,
  ADD COLUMN IF NOT EXISTS variance_value NUMERIC(15,2) DEFAULT 0;
