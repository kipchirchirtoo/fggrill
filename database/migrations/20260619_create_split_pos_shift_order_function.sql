-- Create the POS split-bill RPC expected by the backend.
-- splitShiftOrder() in outlet-pos.controller.ts calls this so that all child
-- bill inserts and the parent bill update happen atomically: a failure
-- partway through must not leave orphaned child bills alongside a still-open
-- parent. Without this function the split-bill endpoint throws at runtime.
...
BEGIN
  ...
  -- The parent bill is fully consumed by its children. kitchen_status is set
  -- to 'served' (not 'cancelled'/'voided') so isKitchenVisiblePosOrder() does
  -- not surface it on the KDS as a void notice — the food was already made
