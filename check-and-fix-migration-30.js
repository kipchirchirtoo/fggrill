...
async function fixMigration30() {
  ...
  try {
    ...
    // 4. Check if payroll table exists
    console.log('4️⃣  Checking payroll table...');
    ...
    if (payrollCheck.rows[0].exists) {
      console.log('   ℹ️  Payroll table exists, adding columns...');
      await client.query(`
        ALTER TABLE payroll 
      ...
      console.log('   ⚠️  Payroll table does not exist, skipping\n');
    }
    ...
    if (supplierCheck.rows[0].exists) {
      ...
      console.log('   ⚠️  Suppliers table does not exist, skipping\n');
    }
    
    // 6. Create supplier_products
    console.log('6️⃣  Creating supplier_products table...');
    const supplierProductsCheck = await client.query(`
      SELECT EXISTS (
        SELECT FROM information_schema.tables 
        WHERE table_name = 'suppliers'
      )
    ...
    if (supplierProductsCheck.rows[0].exists) {
      await client.query(`
      ...
      console.log('   ⚠️  Suppliers table does not exist, skipping supplier_products\n');
    }
